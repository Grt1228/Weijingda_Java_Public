...
public class HttpClientUtil{
	...
	private static HttpResponse execute(HttpConfig config) throws HttpProcessException {
		...
		try {
			...
			RequestConfig requestConfig = RequestConfig.custom().setSocketTimeout(7500).build();
			...
			//config.client().getConnectionManager().releaseConnection();
			//config.client().getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT,60000);
			//config.client().getHttpConnectionManager().getParams().setSoTimeout(2000);
	...
	public static InputStream getInputStream(HttpConfig config){
		InputStream response = null;
		try {
			 response = execute(config).getEntity().getContent();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return response;
	...
	public static HttpResponse getHttpResponse(HttpConfig config){
